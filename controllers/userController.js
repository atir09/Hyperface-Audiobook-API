...
// Retrieving User information and their Courses marked as favorite
...
const addFavoriteCourse = async (req, res) => {
  try {
    ...
    res.status(200).send({message:"Course Added to Favourite",user});
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};
...
const removeFavoriteCourse = async (req, res) => {
  try {
    ...
    res.status(200).send({message:"Course removed from Favourites",user});
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};
